...
class Helpers {
    ...
    static Pipe getRepositoryIssueComments(Vertex repo) {
        return repo.out(EdgeType.ISSUE). \
                    out(EdgeType.ISSUECOMMENT).dedup()
    }
    ...
    static Pipe getRepositoryMergedPullRequests(Vertex repo) {
        return repo.out(EdgeType.PULLREQUEST).hasNot(PropertyName.MERGED_AT, null).dedup()
    }
